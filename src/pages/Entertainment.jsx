    ...
    const getData = async () => {
      try {
        let { data } = await axios(
          `${END_POINT}category=entertainment&q=${search}&pageSize=${PAGE_LIMIT}&page=${page}&apiKey=${API_KEY}`
        );
        let { data: totalData } = await axios(
          `${END_POINT}category=entertainment&q=${search}&apiKey=${API_KEY}`
        );
  ...
  return (
    <Fragment>
      ...
      <section>
        <div className="container">
          <h1
            style={{ textAlign: "center", color: "#191919", fontSize: "20px", paddingBottom: "20px", }} >
            Total news: {total}
          </h1>
